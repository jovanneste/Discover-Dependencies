...
/*
This is my own work except that I used some code found of the following websites in the HashMap struct: 

https://stackoverflow.com/questions/34016439/overloading-the-class-subscript-operator-to-access-elements-of-a-member-stdvec
...
struct HashMap {
  ...
  auto insert(std::pair<std::string, std::list<std::string>> string_tuple){
    std::unique_lock<std::mutex> lock(mtx);
    return theTable.insert(string_tuple);
  }
...
HashMap threadSafeTheTable;
WorkQueue threadSafeworkQ;
//std::unordered_map<std::string, std::list<std::string>> theTable;
//std::list<std::string> workQ;
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (threadSafeTheTable.find(name) != threadSafeTheTable.end()) { continue; }
    // ... insert mapping from file name to empty list in table ...
    threadSafeTheTable.insert( { name, {} } );
    // ... append file name to workQ
    threadSafeworkQ.push_back( name );
  }
  // 3. close file
  fclose(fd);
}
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = &threadSafeTheTable[name];
...
int main(int argc, char *argv[]) {
  ...
  auto no_of_threads = 2;
  // if explore CRAWLER_THREADS = no_of_threads
  if(cthreads) 
    no_of_threads = atoi(cthreads);
  ...
  for (i = start; i < argc; i++) {
    ...
    threadSafeTheTable.insert( { obj, { argv[i] } } );
    
    // 3b. insert mapping from file.ext to empty list
    threadSafeTheTable.insert( { argv[i], { } } );
    
    // 3c. append file.ext on workQ
    threadSafeworkQ.push_back( argv[i] );
  }



  std::thread threads[no_of_threads];
  for (int i = 0; i < no_of_threads; i++) {
    // create a thread
    threads[i] = std::thread([] {
      // 4. for each file on the workQ
      while ( threadSafeworkQ.size() > 0 ) {
        std::string filename = threadSafeworkQ.pop_front();
        
        // 4a&b. lookup dependencies and invoke 'process'
        process(filename.c_str(), &threadSafeTheTable[filename]);
      }
    });
  }
